...
const GKAITutor = (() => {
  ...
  async function respond(input, context = {}, persona = 'student') {
  ...
  async function callGemini(prompt, context = {}, persona = 'student') {
    ...
    if (persona === 'mentor') {
        richerContext = `
[MENTOR CONTEXT]
Mentor: ${context.mentorName || 'Guru'}
Reviewing Student: ${context.studentName || 'Shishya'}
Target Topic: ${context.topicId || 'N/A'}
AQ (Academic): ${context.aq || 'N/A'} / 100
SQ (Spiritual): ${context.sq || 'N/A'} / 100
PQ (Physical): ${context.pq || 'N/A'} / 100
EQ (Emotional): ${context.eq || 'N/A'} / 100
Mood History: ${context.moods ? context.moods.join(', ') : 'N/A'}
Recent Feedback: ${context.lastFeedback || 'N/A'}
...
Subject: ${context.subject || 'Learning'}
Topic: ${context.topic || 'Exploring'}
Subtopic: ${context.subtopic || 'General'}
Current Mood: ${context.mood || 'Balanced'}
Brain Battery: ${context.energy || 80}%
Total XP: ${context.xp || 0}
Completed Milestone Topics: ${context.completedTopics ? context.completedTopics.join(', ') : 'None yet'}
Persona Traits: ${context.persona || 'Standard'}
`;
    }
