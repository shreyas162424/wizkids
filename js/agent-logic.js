...
window.GKAgent = (() => {
    ...
    let _studentAssistantId = 'krishna';

    function setStudentAssistant(id) {
        _studentAssistantId = id || 'krishna';
    }

    function _studentAvatarPath(expression) {
        if (typeof GKAssistant !== 'undefined') {
            const url = GKAssistant.avatarUrlForExpression(expression);
            return url.src;
        }
        const persona = PERSONAS.student;
        return persona.expressions[expression] || persona.expressions.default;
    }
    ...
    return {
        ...
        getAvatar: () => {
            if (_currentPersona === 'student') {
                return _studentAvatarPath(_currentExpression);
            }
